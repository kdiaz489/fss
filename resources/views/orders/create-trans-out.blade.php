...
<script>
    ...
    $(document).ready(function(){
        ...
        function dynamic_field(number){
            ...
            html += '@if (count($cases) > 0) <optgroup label="Cases"> @foreach ($cases as $case) @if($case->total_qty !=0) <option value="{{$case->id}}">{{$case->sku}}</option>@endif @endforeach</optgroup> @else<option value="" disabled>No Cases Available</option> @endif '
            html += '@if (count($kits) > 0) <optgroup label="Kits"> @foreach ($kits as $kit) @if($kit->total_qty !=0) <option value="{{$kit->id}}">{{$kit->sku}}</option>@endif @endforeach</optgroup> @else<option value="" disabled>No Kits Available</option> @endif '
            html += '@if (count($units) > 0) <optgroup label="Units"> @foreach ($units as $unit) @if($unit->total_qty !=0) <option value="{{$unit->id}}">{{$unit->sku}}</option>@endif @endforeach</optgroup> @else<option value="" disabled>No Units Available</option> @endif '
            html += '</select></td>'
